...
# Importe tous nos nouveaux outils
from outils import config
from outils.log import log
from outils.memoire import MemoireCognitive
from outils.securite import execute_commande_securisee
from outils.recherche_web import web_research
from outils.apprentissage import (
...
class GPT2:
    def __init__(self):
        # Initialise l'environnement et les outils au moment de la création de l'objet
        config.setup_environment()
        self.log = log
        self.memoire_cognitive = MemoireCognitive(log_func=self.log)
        self.apprentissage_contextuel = ApprentissageContextuel(log_func=self.log)
        self.detecteur_patterns = DetecteurPatternsAvance(log_func=self.log)
        self.analyseur_correlations = AnalyseurCorrelations(log_func=self.log)

        # Configure l'instance globale de la mémoire pour le décorateur
        from outils.memoire import set_global_memoire_instance
        set_global_memoire_instance(self.memoire_cognitive)
    ...
    def generate(
        ...
        """
        Point d'entrée principal de l'agent. Décide s'il faut générer du texte,
        chercher sur le web ou exécuter une commande.
        """
        # --- Cerveau Décisionnel ---
        if prompt.startswith('!'):
            command = prompt[1:].strip()
            self.log(f"⚡️ Commande détectée: '{command}'")
            result = execute_commande_securisee(
                command, self.log, self.memoire_cognitive, self.apprentissage_contextuel,
                self.detecteur_patterns, self.analyseur_correlations
            )
            print(f"--- Résultat de la commande ---\n{result}")
            return # Termine l'exécution après la commande

        elif "cherche sur le web" in prompt.lower() or "trouve des infos sur" in prompt.lower():
            topic = prompt.lower().split("sur")[-1].strip()
            self.log(f"🛰️ Recherche web détectée sur: '{topic}'")
            search_results = web_research(topic, self.log)

            if not search_results:
                knowledge = "Je n'ai rien trouvé sur le web à ce sujet."
            else:
                knowledge = "\n\n".join(
                    f"--- Source: {site} ---\n{content}"
                    for site, content in search_results.items()
                )

            prompt = f"Contexte de la recherche web:\n{knowledge}\n\nBasé sur ce contexte, réponds à la question: {prompt}"
            self.log(f"Injecting web research context into prompt.")
            # La logique continue vers la génération de texte standard avec ce nouveau prompt enrichi

        # --- Logique de Génération de Texte (existante) ---
        ...
        with tf.Session(graph=tf.Graph()) as sess:
            ...
            if not prompt:
                ...
                # La logique RAG est maintenant un cas particulier de la génération standard
                if rag:
                    knowledge = self._retrieve_knowledge(prompt)
                    prompt = f"Contexte: {knowledge}\n\nQuestion: {prompt}\n\nRéponse:"
