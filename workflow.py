...
"""
Système de Workflow Automatisé pour Jules - Version Raffinée
...
def start_daemon():
    """Lance le démon en tant que processus détaché."""
    if PID_FILE.exists():
        try:
    ...
    command = [sys.executable, __file__, 'daemon', '--run-loop']
...
def daemon_run_loop(config: WorkflowConfig):
    """La boucle de travail actuelle du démon. Appelée en interne."""
    # Écrire le fichier PID
    try:
        PID_FILE.write_text(str(os.getpid()))
    except IOError as e:
        # Ne peut pas logger car le logging n'est pas encore configuré
        sys.exit(f"Erreur critique: Impossible d'écrire le fichier PID: {e}")

    # Enregistrer le nettoyage
    import atexit
    atexit.register(cleanup_pid_file)

    # Configurer la journalisation et démarrer la boucle
    setup_logging(config.log_level)
    log_activity("Démon démarré", f"PID: {os.getpid()}")
    daemon_loop(config)
...
def main():
    ...
    daemon_group.add_argument("--run-loop", action="store_true", help=argparse.SUPPRESS) # Caché
...
if __name__ == "__main__":
    ensure_memory_directory()
    # Le logging est géré dans main() pour la plupart des commandes
    # ou dans daemon_run_loop() pour le démon.
    main()
