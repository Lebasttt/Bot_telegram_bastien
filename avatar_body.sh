...
update_gist() {
    ...
    response_code=$(curl -s -L -o /dev/null -w "%{http_code}" -X PATCH \
    ...
    if [[ "$response_code" -eq 404 ]]; then
        log "ERREUR CRITIQUE: Le Gist avec l'ID '$GIST_ID' n'a pas été trouvé. Vérifie que le GIST_ID est correct et que ton GITHUB_TOKEN y a accès."
    elif [[ "$response_code" -ne 200 ]]; then
        log "ERREUR: La mise à jour du Gist a échoué (Code HTTP: $response_code)"
    fi
}
...
while true; do
    ...
    if [ -n "$jules_command" ]; then
        log "Exécution directe de la commande reçue."
